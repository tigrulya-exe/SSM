...
public class CmdletManager extends AbstractService {
  ...
  private final List<Long> pendingCmdlet;
  private final List<Long> schedulingCmdlet;
  private final Queue<Long> scheduledCmdlet;
  private final Map<Long, LaunchCmdlet> idToLaunchCmdlet;
  private final List<Long> runningCmdlets;
  private final Map<Long, CmdletInfo> idToCmdlets;
  // Track a CmdletDescriptor from the submission to
  // the finish.
  private final TaskTracker tracker;
  private final Map<Long, ActionInfo> idToActions;
  private final Map<Long, CmdletInfo> cacheCmd;
  private final List<Long> toBeDeletedCmd;
  private final ListMultimap<String, ActionScheduler> schedulers = ArrayListMultimap.create();
  private final CmdletLifecycleListener lifecycleListener;
  private List<ActionSchedulerService> schedulerServices = new ArrayList<>();

  private final AtomicLong numCmdletsGen = new AtomicLong(0);
  private final AtomicLong numCmdletsFinished = new AtomicLong(0);

  private ActionGroup tmpActions = new ActionGroup();

  private long timeout;

  public CmdletManager(ServerContext context, CmdletLifecycleListener lifecycleListener) throws IOException {
    super(context);
  ...
  private void reloadCmdletsInDB() throws IOException {
    ...
    try {
      ...
      if (cmdletInfos != null && !cmdletInfos.isEmpty()) {
      ...
      if (cmdletInfos != null && !cmdletInfos.isEmpty()) {
        for (CmdletInfo cmdletInfo : cmdletInfos) {
          ...
          tracker.track(cmdletInfo.getCid(), cmdletDescriptor);
          LOG.debug("Reload pending cmdlet: {}", cmdletInfo);
          List<ActionInfo> actionInfos = getActions(cmdletInfo.getAids());
          syncCmdAction(cmdletInfo, actionInfos);
        }
      }
  ...
  public long submitCmdlet(String cmdlet) throws IOException {
    ...
    try {
      ...
      // todo log only cmdlets added by user or not
      long cmdletId = submitCmdlet(cmdletDescriptor);
      lifecycleListener.cmdletAdded(cmdletId);
      return cmdletId;
  ...
  private void batchSyncCmdAction() {
    if (cacheCmd.isEmpty() && toBeDeletedCmd.isEmpty()) {
      return;
    }
    ...
    if (!cmdletInfos.isEmpty()) {
      ...
      try {
        ...
        metaStore.insertCmdlets(
            cmdletInfos.toArray(new CmdletInfo[0]));
      } catch (MetaStoreException e) {
    ...
    if (todelSize > 0) {
      ...
      synchronized (toBeDeletedCmd) {
        del.addAll(toBeDeletedCmd.subList(0, Math.min(todelSize, cacheCmdTh)));
  ...
  private boolean shouldStopSchedule() {
    ...
    return scheduledCmdlet.size() >= left + total * 0.2;
  }
  ...
  private int scheduleCmdlet() {
    int nScheduled = 0;

    synchronized (pendingCmdlet) {
      if (!pendingCmdlet.isEmpty()) {
  ...
  private ScheduleResult scheduleCmdletActions(CmdletInfo info, LaunchCmdlet launchCmdlet) {
    ...
    for (idx = 0; idx < actIds.size(); idx++) {
      ...
      if (actSchedulers == null || actSchedulers.isEmpty()) {
        skipped = true;
        continue;
      }
  ...
  private LaunchCmdlet createLaunchCmdlet(CmdletInfo cmdletInfo) {
    ...
    for (Long aid : cmdletInfo.getAids()) {
      if (idToActions.containsKey(aid)) {
        ...
        args = new HashMap<>(toLaunch.getArgs());
        launchActions.add(
  ...
  public void activateCmdlet(long cid) {
  ...
  private void cmdletFinished(long cmdletId) {
  ...
  public void deleteCmdlet(long cmdletId) throws IOException {
    ...
    try {
      ...
      lifecycleListener.cmdletDeleted(cmdletId);
    } catch (MetaStoreException e) {
  ...
  public void deleteCmdletByRule(long rid) throws IOException {
    ...
    if (cmdletInfoList == null || cmdletInfoList.isEmpty()) {
      return;
    }
    List<Long> cids = new ArrayList<>();
  ...
  private class CmdletPurgeTask implements Runnable {
    ...
    private final long lifeCheckInterval;
  ...
  private class DetectFailedActionTask implements Runnable {
    ...
    public void run() {
      try {
        List<Long> cids = new ArrayList<>(idToLaunchCmdlet.keySet());
        for (Long cid : cids) {
