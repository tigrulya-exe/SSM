...
import org.smartdata.http.config.ldap.search.group.GroupSearchByMemberAttrFactory;
...
public class TestGroupSearchByMemberAttrFactory extends TestLdapSearchTemplateFactory {

  @Override
  protected LdapSearchTemplateFactory create(SmartConf conf) {
    return new GroupSearchByMemberAttrFactory(conf);
  }
  ...
  public void checkGeneratedQueryWithDefaultConf() {
    ...
    String expectedTemplate =
        "(&(objectClass=groupOfNames)(|(cn=group1)(cn=group2))(member={0}))";
  ...
  public void checkGeneratedQueryWithCustomConf() {
    SmartConf conf = new SmartConf();
    conf.set(SMART_REST_SERVER_LDAP_USER_GROUPS, "group3");
    conf.set(SMART_REST_SERVER_LDAP_GROUP_NAME_ATTR, "guid");
    conf.set(SMART_REST_SERVER_LDAP_GROUP_MEMBER_ATTR, "mmb");

    String expectedTemplate =
        "(&(objectClass=groupOfNames)(guid=group3)(mmb={0}))";
