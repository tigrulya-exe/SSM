...
import org.smartdata.server.mappers.CachedFileMapper;
import org.smartdata.server.mappers.pagination.CachedFilePageRequestMapper;
...
public class CachedFilesControllerDelegate {
  ...
  public CachedFilesDto getCachedFiles(PageRequestDto pageRequestDto,
    ...
    CachedFileSearchRequest searchRequest = cachedFileMapper.toSearchRequest(
        pathLike, lastAccessedTime, cachedTime);

    return cachedFileMapper.toCachedFiles(
        cachedFilesManager.search(searchRequest, pageRequest));
  }
}
