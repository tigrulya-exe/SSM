...
public class ActionsControllerDelegate implements ActionsApiDelegate {
  ...
  public ActionsDto getActions(
      ...
      List<@Valid ActionSortDto> actionSort,
  ...
  public ActionInfoDto submitAction(
    ...
    String action = submitActionRequestDto.getAction();
    CmdletInfo cmdletInfo = cmdletManager.submitCmdlet(action);

    long actionId = Optional.ofNullable(cmdletInfo.getAids())
        .filter(CollectionUtils::isNotEmpty)
        .map(actionIds -> actionIds.get(0))
        .orElseThrow(() -> new RuntimeException("Cmdlet doesn't generate actions: " + action));
